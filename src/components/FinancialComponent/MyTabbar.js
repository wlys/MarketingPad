...
var MyTabbar = React.createClass({
    ...
    _tabSelectedEvent(i) {
        ...
        if(1>this._markArray[i]){
            ...
            if(this._topRouter){
                this._navigator.jumpTo(this._topRouter);
            }
            console.log("this._topRouter; " + this._topRouter);
            this._topRouter = {name:i};
            this._navigator.push(this._topRouter);
        }else{
            console.log("this._navigator.jump({name:i});" + i);
            this._navigator.jumpTo(this._routerArray[i]);
        }
            ...
            <View>
                <View style={styles.flexContainer}>
                    <ScrollView key={'scrollView'} horizontal={true}>
                        { this._renderTabHeader(this.props.tabArray,styles)}
                    </ScrollView>
